    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 添加积分商品
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            <el-form-item label="商品分类" prop="type">
              <el-select placeholder="选择分类" v-model="form.type" clearble>
                <!-- <el-option v-for="(item,index) in cateList" :key="index" :label="item.name" :value="item.id"></el-option> -->
                <el-option label="现金商品" :value="1"></el-option>
                <el-option label="实物商品" :value="2"></el-option>
              </el-select>
            </el-form-item>
            <!--现金上商品独有-->
            <el-form-item label="现金面额" prop="bills" v-if="form.type == 1">
              <el-input type="number" v-model="form.bills"></el-input>
            </el-form-item>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
              ...
              bills:[
                { required: true, message: '请输入现金面额', trigger: 'change' },
              ],
              goodsName:[
    ...
    methods: {
      ...
      //获取上详情
      getGoodsDetail(){
        fetchData(`/xy-shop-goods-list/find?id=${this.editId}`).then(res => {
          this.form.type = res.data.type
          this.form.bills = res.data.bills
      ...
      onSubmit(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(this.editId){
              updateData(`/xy-shop-goods-list/update`,this.form).then(res => {
                this.$message.success('操作成功')
                this.$router.go(-1)
              });
            }else{
              postData(`/xy-shop-goods-list/add`,this.form).then(res => {
                this.$message.success('操作成功')
                this.$router.go(-1)
              });
            }
